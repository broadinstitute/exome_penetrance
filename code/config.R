...
read_google_tsv <- function(remote_path, ...) {
  ...
  try({
    hash = system(paste0("/Users/jgoodric/google-cloud-sdk/bin/gsutil hash -hm ", remote_path, " | cut -f4 | tail -1"), intern=TRUE)
...
carrier_categories = c("Carrier Pathogenic",
                       "Carrier Likely Pathogenic",
                       "Carrier VUS",
                       "Carrier Likely Benign",
                       "Carrier Benign",
                       "Carrier LOF",
                       "Carrier Likely_LOF",
                       "Carrier LOF_Unknown",
                       "Carrier Likely_Not_LOF",
                       "Carrier Not_LOF",
                       "Non Carrier")

high_conf_lab = c("Invitae",
                  "GeneDx",
...
get_vcf_path = function(reference){
  vcf_path = paste("../data/ClinVar/clinvar_20190408_",reference,".vcf.gz",sep ="")
}
...
get_sum_clinvar_sub_by_gene_prefix = function(genome_build){
  return(paste(clinvar_output_path,"/sum_all_submissions_and_variants_by_gene_",genome_build,"_",sep = ""))
}
...
get_data_path = function(data_source, data_type){
  ...
  if (data_type == "gnomad_AF"){
    return(paste(gnomad_AF_path,paste(data_source,"_gnomad_AF.txt",sep=""), sep = "/"))
  }
...
disease_order = c("High LDL", "Low LDL","High HDL","High triglycerides","Monogenic obesity","MODY","MODY extended","Lipodystrophy","Neonatal diabetes")
threshold_for_plot = data.frame(Trait=c("High HDL","High LDL","Low LDL","High triglycerides","Obesity"),
                                threshold = c(70,190,80,200,30),stringsAsFactors = F)

variant_levels = c("NC","C","Non Carrier","Carrier","ClinVar","LOF","Review", "ClinVar and LOF",
                   "LOF and Review","VUS Novel","VUS <1/10000","VUS <1/1000","VUS >=1/1000")

colors <- c("Review"="#5B9555",
            "ClinVar"="#377eb8",
            "LOF"="#d6604d",
            "Non Carrier"="#666666",
            "Carrier"= "#d6604d",
            "NC"="#666666",
            "C"= "#d6604d",
            "ClinVar and LOF"="#807dba",
            "LOF and Review"="#e88c30",
            "VUS Novel" = "#2b8cbe",
            "VUS <1/10000" = "#80b1d3",
            "VUS <1/1000" = "#fdb462",
            "VUS >=1/1000" = "#b3de69")
...
get_PRS_path = function(phenotype){
  PRS_fp = switch(phenotype, 
         "ldl_mgdl" =  "../data/PRS/LDL_PRS_ukb_v3_imp_info_0.3_inf_ENGAGE_hm3_h2_0.1347_score.txt.profile",
         "hdl_mgdl" = "../data/PRS/HDL_PRS_ukb_v3_imp_info_0.3_inf_ENGAGE_hm3_h2_0.1572_score.txt.profile",
         "tg_mgdl" = "../data/PRS/TG_PRS_ukb_v3_imp_info_0.3_inf_ENGAGE_hm3_h2_0.1572_score.txt.profile",
         "tg_mgdl_ln" = "../data/PRS/TG_PRS_ukb_v3_imp_info_0.3_inf_ENGAGE_hm3_h2_0.1572_score.txt.profile",
         "bmi" =  "../data/PRS/penetrance_ukb_imp_chr_Khera.et.al_GPS_BMI_Cell_2019_mod_fix_import.tsv",
         "bmi_ln" =  "../data/PRS/penetrance_ukb_imp_chr_Khera.et.al_GPS_BMI_Cell_2019_mod_fix_import.tsv",
         "t2d" =  "../data/PRS/penetrance_ukb_imp_chr_Type2Diabetes_PRS_LDpred_rho0.01_v3_fix_import.tsv")
  return(PRS_fp)
}
